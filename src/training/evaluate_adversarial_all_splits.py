"""
Evaluate adversarial debiasing model on multiple balanced splits.
Compares adversarial model bias metrics against baseline.
"""
...
from models.adv_model_dual import create_dual_adversarial_model
...
def format_multi_split_table(all_results: Dict[str, Dict]) -> str:
    """
    Format results from multiple splits into a table.
    """
    lines = []
    lines.append("=" * 120)
    lines.append("ADVERSARIAL MODEL - BIAS EVALUATION RESULTS")
    lines.append("=" * 120)
    lines.append("")
    ...
    lines.append("")
    lines.append("EXPECTED ADVERSARIAL TRAINING RESULTS:")
    lines.append("  - Δ should DECREASE compared to baseline (bias reduction)")
    lines.append("  - F1 may slightly decrease (acceptable trade-off)")
    lines.append("  - λM should approach λD(F1) (model becomes race-agnostic)")
    lines.append("=" * 120)
...
def main():
    ...
    parser = argparse.ArgumentParser(description='Evaluate adversarial model on multiple splits')
    ...
    parser.add_argument('--use_val', action='store_true',
                       help='Evaluate on validation set instead of training set')
    ...
    status.expect_partial()  # Ignore optimizer/epoch variables
    ...
    # Save JSON
    json_file = os.path.join(output_dir, f'adversarial_all_splits_{timestamp}.json')
    ...
    table_file = os.path.join(output_dir, f'adversarial_table_all_splits_{timestamp}.txt')
